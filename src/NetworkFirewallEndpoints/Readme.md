...
WORKDIR /src
COPY ["NetworkFirewallEndpoints.csproj", "NetworkFirewallEndpoints/"]
RUN dotnet restore "NetworkFirewallEndpoints/NetworkFirewallEndpoints.csproj"

WORKDIR "/src/NetworkFirewallEndpoints"
COPY . .
RUN dotnet build "NetworkFirewallEndpoints.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "NetworkFirewallEndpoints.csproj" \
...
Execute unit tests
```
    cd "NetworkFirewallEndpoints/test/NetworkFirewallEndpoints.Tests"
    dotnet test
```

Deploy application
```
    cd "NetworkFirewallEndpoints/src/NetworkFirewallEndpoints"
